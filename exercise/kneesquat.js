...
function setup() {
  ...
  const modelInfo = {
    model: 'exercise/kneesquat/model.json',
    metadata:'exercise/kneesquat/model_meta.json',
    weights:'exercise/kneesquat/model.weights.bin',
  };
...
function modelReady() {
  ...
  setTimeout(() => speakText("squat down and stand up", 1, 0.75), 10000);
}
...
function updateCount(poseLabel){
  ...
  if (lastLabel === 'STAND' && poseLabel === 'SQUAT'){
    count++;
