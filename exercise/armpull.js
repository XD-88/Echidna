...
function setup() {
  ...
  const modelInfo = {
    model: 'exercise/armpull/model.json',
    metadata:'exercise/armpull/model_meta.json',
    weights:'exercise/armpull/model.weights.bin',
  };
...
function modelReady() {
  ...
  setTimeout(() => speakText("pull your arms back", 1, 0.75), 10000);
}
...
function updateCount(poseLabel){
  ...
  if (lastLabel === 'EXTEND' && poseLabel === 'PULL'){
    count++;
