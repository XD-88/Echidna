...
function setup() {
  ...
  const modelInfo = {
    model: 'exercise/standinghipabductionleft/model.json',
    metadata:'exercise/standinghipabductionleft/model_meta.json',
    weights:'exercise/standinghipabductionleft/model.weights.bin',
  };
...
function modelReady() {
  ...
  setTimeout(() => speakText("lift your left leg up sideways and put it down", 1, 0.75), 10000);
}
...
function draw() {
  ...
  text(poseLabel,23*width/24,7*height/8);
...
function updateCount(poseLabel){
  ...
  if (lastLabel === 'DOWN' && poseLabel === 'UP'){
    count++;
