...
function setup() {
  ...
  const modelInfo = {
    model: 'exercise/sittostand/model.json',
    metadata:'exercise/sittostand/model_meta.json',
    weights:'exercise/sittostand/model.weights.bin',
  };
...
function modelReady() {
  ...
  setTimeout(() => speakText("sit down on the chair and stand up", 1, 0.75), 10000);
  console.log("Spoken");
}
...
function updateCount(poseLabel){
  ...
  if (lastLabel === 'STAND' && poseLabel === 'SIT'){
    count++;
